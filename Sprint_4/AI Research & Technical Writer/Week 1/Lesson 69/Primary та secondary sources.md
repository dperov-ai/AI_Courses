# Урок 69: Первинні (Primary), вторинні (Secondary) та третинні джерела інформації: Ієрархія доказовості

## 1. Фундаментальна концепція та піраміда доказовості

В епоху інформаційного шуму та генеративного контенту здатність розрізняти природу походження даних є головною компетенцією Research Analyst та Technical Writer. Будь-який факт у технічній документації повинен спиратися на надійне першоджерело, а не на переказ переказу в сумнівному блозі.

```mermaid
flowchart BT
    subgraph EvidencePyramid [Ієрархія джерел інформації]
        direction BT
        Primary["1. ПЕРВИННІ ДЖЕРЕЛА (Primary Sources)\nОфіційні RFC стандарти, вихідний код, сирі логи, патенти, експерименти"]
        Secondary["2. ВТОРИННІ ДЖЕРЕЛА (Secondary Sources)\nРецензовані статті, технічні огляди, аналітичні звіти Gartner/IDC"]
        Tertiary["3. ТРЕТИННІ ДЖЕРЕЛА (Tertiary Sources)\nЕнциклопедії (Wikipedia), підручники, агрегатори новин"]
...
## 2. Детальна класифікація джерел

| Тип джерела | Визначення | Приклади в IT та науці | Ступінь довіри |
| :--- | :--- | :--- | :--- |
| **Первинні (Primary)** | Прямі, необроблені свідчення автора, артефакти або оригінальні дані експерименту. | Специфікація W3C/IETF, Git-коміт розробника, аудіозапис інтерв'ю, датасет бенчмарку. | **Найвища (Еталон)** |
| **Вторинні (Secondary)** | Аналіз, інтерпретація, критика або узагальнення первинних джерел. | Стаття в науковому журналі, що аналізує 10 алгоритмів, огляд трендів від Gartner. | **Висока (Потребує звірки)** |
| **Третинні (Tertiary)** | Збірники, покажчики та компіляції первинних і вторинних матеріалів. | Статті у Вікіпедії, словники термінів, каталоги бібліотек. | **Орієнтовна (Не для цитування в whitepapers)** |

---

## 3. Практична лабораторія та челенджі

### Лабораторне завдання 1: Трасування інформаційного ланцюжка до першоджерела
**Мета:** Вам надано новину на популярному техно-сайті: *«Новий алгоритм стиснення зменшує трафік на 90%»*.
1. Знайти вторинне джерело (прес-реліз компанії).
2. Знайти первинне джерело (наукова стаття на arXiv або репозиторій на GitHub).
3. Порівняти реальні результати бенчмарку з гучним заголовком новини та виявити маніпуляції.

---

## 4. Підсумковий чекліст компетенцій та глосарій

- [ ] Я можу точно класифікувати джерело як первинне, вторинне чи третинне.
- [ ] Я знаю, як знайти оригінальний RFC стандарт або вихідний код замість читання чужих інтерпретацій.
- [ ] Я вмію будувати ланцюжки верифікації фактів.

### Глосарій термінів:
- **Primary Source (Першоджерело):** Документ або дані, створені безпосередньо в момент події чи дослідником, який отримав результат.
- **Peer Review (Рецензування):** Процес незалежної експертної оцінки наукової статті іншими вченими перед її публікацією.
---

## 8. Поглиблений інженерний аналіз та низькорівнева архітектура

Для досягнення рівня Senior Engineer та системного розуміння концепції **«Primary та secondary sources»**, необхідно проаналізувати, як ці механізми взаємодіють з операційною системою, ядрами процесора, пам'яттю та розподіленими мережами.
...
```mermaid
flowchart TD
    AppLayer["Прикладний рівень програми: Логіка 'Primary та secondary sources'"] --> RuntimeVM["Віртуальна машина / Середовище виконання (CPython, V8, JVM, Go Runtime)"]
...
Розглянемо практичну реалізацію промислового стандарту для концепції **«Primary та secondary sources»** з урахуванням надійності, відмовостійкості та високої продуктивності.
...
class ProductionEngine(BaseEngineInterface[Dict[str, Any]]):
    ...
    def execute(self, payload: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
        ...
        logger.info(f"[{context.request_id}] Початок обробки в контексті 'Primary та secondary sources'")
        ...
        try:
            processed_data = {
                ...
                "execution_trace": f"Engine applied standard patterns for Primary та secondary sources"
            }
...
### 10.1. Структурований системний промпт для теми «Primary та secondary sources»
```markdown
### SYSTEM PERSONA:
Ти — провідний Principal Architect та Domain Expert з 15-річним досвідом у High-Load системах та забезпеченні якості.
Твоя спеціалізація: глибока експертиза в темі «Primary та secondary sources».
...
Коли система обробляє понад 1 000 000 запитів на секунду, будь-яка недбалість у реалізації **«Primary та secondary sources»** призводить до ефекту каскадної відмови (Cascading Failure):
...
### Лабораторний проект рівня Middle+: Побудова виробничого модуля «Primary та secondary sources»
...
- [ ] Я можу пояснити концепцію «Primary та secondary sources» простими словами для джуніора та на рівні системної архітектури для CTO.
